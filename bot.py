...
async def track_cooldown_smart(ctx, cmd):
    ...
    emoji = cooldown_emojis.get(cmd, "⏰")
    is_short_cooldown = cmd == "mission"
    
    if remaining > 0:
        time_str = format_time(remaining)
        
        if is_short_cooldown:
            await ctx.send(f"{emoji} **{cmd.upper()}** on cooldown! **{time_str}** remaining.")
            return
    ...
    if is_short_cooldown:
        wait_msg = await ctx.send(f"{emoji} Checking Naruto Botto...")
    else:
        embed = discord.Embed(
    ...
    if user_id in cooldowns and cmd in cooldowns[user_id]:
        ...
        if cooldown_data["expires_at"] > time.time():
            ...
            if is_short_cooldown:
                await wait_msg.edit(content=f"{emoji} **{cmd.upper()}** tracked! **{time_str}** remaining. I'll ping you when ready!")
                return
    ...
    if is_short_cooldown:
        await wait_msg.edit(content=f"{emoji} **{cmd.upper()}** started! **{duration}** cooldown. I'll ping you when ready!")
        return
